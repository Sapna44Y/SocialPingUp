        ...
        <div className="mb-8 flex flex-wrap gap-6">
          {dataArray.map((item, index) => (
            ...
            >
              ...
              <p className="text-slate-60">{item.label}</p>
            </div>
          ))}
        </div>
        ...
        <div className="inline-flex flex-wrap items-center border border-gray-200 rounded-md p-1 bg-white shadow-sm">
          {dataArray.map((tab) => (
            <button
              ...
              key={tab.label}
              className={`cursor-pointer flex items-center px-3 py-1 text-sm rounded-md transition-colors ${currentTab === tab.label ? "bg-white front-medium text-black" : "text-gray-500 hover:text-black"}`}
            >
